...
arrays = [
    ...
    np.random.randint(0, 255, size=10 * 11 * 12, dtype='u1').reshape(10, 11, 12, order='F'),
    np.random.randint(0, 255, size=10 * 11 * 12, dtype='u1').reshape(10, 11, 12, order='C'),
]
...
def test_planarization():
    for array, codec in product(arrays, codecs):
        if array.ndim > 2: 
            assert codec.encode(array) == codec.encode(array.reshape(array.shape[0] * array.shape[1], -1))